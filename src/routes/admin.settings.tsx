...
function SettingsAdmin() {
  ...
  useEffect(() => {
    supabase.from("site_settings").select("key,value").in("key", ["contact", "promo_bar", "home"]).then(({ data }) => {
      for (const r of data ?? []) {
  ...
  return (
    <div className="space-y-10 max-w-2xl">
      ...
      <section className="grid sm:grid-cols-2 gap-3">
        <h2 className="font-display text-xl mb-1 sm:col-span-2">Contact info</h2>
        ...
        <div><Label>Instagram URL</Label><Input value={contact.instagram ?? ""} onChange={(e) => setContact({ ...contact, instagram: e.target.value })} /></div>
        <div><Label>Facebook URL</Label><Input value={contact.facebook ?? ""} onChange={(e) => setContact({ ...contact, facebook: e.target.value })} /></div>
      </section>
